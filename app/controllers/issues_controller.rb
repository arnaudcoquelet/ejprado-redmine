...
class IssuesController < ApplicationController
  ...
  def index
    ...
    if @query.valid?
      ...
      respond_to do |format|
        ...
        format.csv  { send_data(issues_to_csv(@issues, @project).read, :type => 'text/csv; header=present', :filename => @project.name + '-' + Date.today.to_s + '-export.csv') }
        format.pdf  { send_data(render(:template => 'issues/index.rfpdf', :layout => false), :type => 'application/pdf', :filename => @project.name + '-' + Date.today.to_s + '-export.pdf') }
      end
    else
  ...
  def show
    ...
    #@allowed_accounting = @issue.new_accounting_statuses_allowed_to(User.current)
	...
	@checklist = Enumeration::get_values('CHEK')
  ...
  def new
	...
	if request.get?
		...
		@checklist = Enumeration::get_values('CHEK')
		@issue.checked_element_names = params[:enabled_checklists]
	end
    ...
    default_accounting = AccountingStatus.default
    ...
    @issue.accounting = default_accounting
    @allowed_statuses = ([default_status] + default_status.find_new_statuses_allowed_to(User.current.role_for_project(@project), @issue.tracker)).uniq
    @allowed_accounting = ([default_accounting] + default_accounting.find_new_accounting_statuses_allowed_to(User.current.role_for_project(@project), @issue.tracker)).uniq

    #Set the Due_To
    @issue.due_date = get_workingdate( Date.today + Tracker.find_by_id(@issue.tracker_id).duration.to_i.days )
    @issue.estimated_hours = 0
    @issue.duestep_date = get_workingdate( Date.today + 1.days )
    @issue.issue_progress = 3
 
    if request.get? || request.xhr?
      @issue.start_date ||= Date.today
    else
      requested_status = IssueStatus.find_by_id(params[:issue][:status_id])
      requested_accounting = AccountingStatus.find_by_id(params[:issue][:accounting_id])
      
      # Check that the user is allowed to apply the requested status
      @issue.status = (@allowed_statuses.include? requested_status) ? requested_status : default_status
      @issue.accounting = (@allowed_accounting.include? requested_accounting) ? requested_accounting : default_accounting
  ...
  def edit
    ...
    @time_entry = TimeEntry.new
    @cost_entry = CostEntry.new
    @current_stepid = @issue.status.id
    ...
    if request.post?
      ...
      #Set Due_To_Step
	  unless attrs.nil?
	      unless @current_stepid.to_s == attrs[:status_id]
	        if(@issue.duestep_date.nil?) 
	          @issue.duestep_date = Date.today
	        end
	        @issue.duestep_date = Date.today + IssueStatus.find_by_id(attrs[:status_id]).duration
	        @issue.duestep_date = get_workingdate(@issue.duestep_date)
	      end
	
		  status = IssueStatus.find_by_id(attrs[:status_id]).is_closed?
	      if( status )
	        @issue.due_date = Date.today
	      end
		  
		  #Get Issue Qualification
	      @issue.issue_progress = @issue.get_qualification()
	      
	      #Set the Progess ratio
	      @issue.done_ratio = IssueStatus.find_by_id(attrs[:status_id]).progress
	  end
	  ...
	  if ((@time_entry.hours.nil? || @time_entry.valid?) || (@cost_entry.costs.nil? || @cost_entry.valid?)) && @issue.save
		
	  if (@time_entry.hours.nil? || @time_entry.valid?)
        # Log spend time
	  ...
	  if (@cost_entry.costs.nil? || @cost_entry.valid?)
        # Log spend time
        if current_role.allowed_to?(:log_cost)
          @cost_entry.save
        end
        if !journal.new_record?
          # Only send notification if something was actually changed
  ...
  def bulk_edit
    if request.post?
      ...
      accounting = params[:accounting_id].blank? ? nil : AccountingStatus.find_by_id(params[:accounting_id])
      ...
      @issues.each do |issue|
        ...
        issue.category = category if category || params[:category_id] == 'none'
		    issue.client_buyer_name = client_buyer_name if client_buyer_name || params[:client_buyer_name] == 'none'
        issue.client_buyer_firtsname = client_buyer_firtsname if client_buyer_firtsname || params[:client_buyer_firtsname] == 'none'
        issue.client_buyer_idnumber = client_buyer_idnumber if client_buyer_idnumber || params[:client_buyer_idnumber] == 'none'
    ...
    # Find potential statuses the user could be allowed to switch issues to
    @available_accounting = Workflow.find(:all, :include => :new_accounting,
                                          :conditions => {:role_id => current_role.id}).collect(&:new_accounting).compact.uniq.sort
  ...
  def destroy
	...
	@costs = CostEntry.sum(:costs, :conditions => ['issue_id IN (?)', @issues]).to_f
  ...
  def context_menu
    ...
    @can = {:edit => (@project && User.current.allowed_to?(:edit_issues, @project)),
            ...
            :log_cost => (@project && User.current.allowed_to?(:log_cost, @project)),			
