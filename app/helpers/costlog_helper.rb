module CostlogHelper
    ...
    CSV::Writer.generate(export, l(:general_csv_separator)) do |csv|
      ...
      entries.each do |entry|
        fields = [l_date(entry.spent_on),
                  ...
                  entry.costs.to_s.gsub('.', decimal_separator),
                  entry.comments
                  ]
  ...
  def report_to_csv(criterias, periods, costs)
    export = StringIO.new
    CSV::Writer.generate(export, l(:general_csv_separator)) do |csv|
      # Column headers
      ...
      # Content
      report_criteria_to_csv(csv, criterias, periods, costs)
      # Total row
      row = [ l(:label_total) ] + [''] * (criterias.size - 1)
      total = 0
      periods.each do |period|
        sum = sum_costs(select_costs(costs, @columns, period.to_s))
        total += sum
        row << (sum > 0 ? "%.2f" % sum : '')
      end
      row << "%.2f" %total
  ...
  def report_criteria_to_csv(csv, criterias, periods, costs, level=0)
    costs.collect {|h| h[criterias[level]].to_s}.uniq.each do |value|
      ...
      total = 0
      periods.each do |period|
        sum = sum_costs(select_costs(costs_for_value, @columns, period.to_s))
        total += sum
        row << (sum > 0 ? "%.2f" % sum : '')
      end
      row << "%.2f" %total
      ...
      if criterias.length > level + 1
        report_criteria_to_csv(csv, criterias, periods, costs_for_value, level + 1)
      end
    end
  end
