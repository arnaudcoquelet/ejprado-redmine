...
class AccountingStatus < ActiveRecord::Base
  ...
  def after_save
    AccountingStatus.update_all("is_default=#{connection.quoted_false}", ['id <> ?', id]) if self.is_default?
  end  
  ...
  def find_new_accounting_statuses_allowed_to(role, tracker)  
  ...
  def new_accouting_status_allowed_to?(status, role, tracker)
    status && role && tracker ?
  ...
  def get_duration(status)
    status.find(:first, :conditions => {:status_id => status.id})
  end

private
  def check_integrity
    raise "Can't delete status" if Issue.find(:first, :conditions => ["accounting_id=?", self.id])
  end
end
