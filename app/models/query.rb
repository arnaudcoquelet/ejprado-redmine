...
class Query < ActiveRecord::Base
  ...
  @@available_columns = [
	  ...
	  QueryColumn.new(:client_buyer_idnumber, :sortable => "#{Issue.table_name}.client_buyer_idnumber"),
    QueryColumn.new(:client_buyer_name, :sortable => "#{Issue.table_name}.client_buyer_name"),
	  QueryColumn.new(:client_buyer_firstname, :sortable => "#{Issue.table_name}.client_buyer_firstname"),
    ...
    QueryColumn.new(:issue_progress, :sortable => "#{Issue.table_name}.issue_progress"),
  ...
  def available_filters
    ...
    @available_filters = { "status_id" => { :type => :list_status, :order => 1, :values => IssueStatus.find(:all, :order => 'position').collect{|s| [s.name, s.id.to_s] } },       
                           ...
                           "Name" => { :type => :text, :order => 3 },
						   "Firstname" => { :type => :text, :order => 4 },
						   "ID Number" => { :type => :text, :order => 5 },
						   "priority_id" => { :type => :list, :order => 6, :values => Enumeration.find(:all, :conditions => ['opt=?','IPRI'], :order => 'position').collect{|s| [s.name, s.id.to_s] } },
                           ...
                           "estimated_hours" => { :type => :integer, :order => 12 },
    ...
    @available_filters["assigned_to_id"] = { :type => :list_optional, :order => 14, :values => user_values } unless user_values.empty?
    @available_filters["author_id"] = { :type => :list, :order => 15, :values => user_values } unless user_values.empty?
  
    if project
      # project specific filters
      unless @project.issue_categories.empty?
        @available_filters["category_id"] = { :type => :list_optional, :order => 16, :values => @project.issue_categories.collect{|s| [s.name, s.id.to_s] } }
      end
      unless @project.active_children.empty?
        @available_filters["subproject_id"] = { :type => :list_subprojects, :order => 17, :values => @project.active_children.collect{|s| [s.name, s.id.to_s] } }
      end
